...
logging.basicConfig(
      ...
      handlers=[
                logging.FileHandler(f"logs/scraper_{datetime.now().strftime('%Y%m%d_%H%M')}.log"),
...
async def process_subscription(
    ...
    try:
        ...
        # Check against target price & historical low
        ...
        if should_notify:
              ...
              logger.info(f"[{sub_id}] No alert needed (reason: {reason})")

except Exception as e:
        logger.error(f"[{sub_id}] Error processing subscription: {e}", exc_info=True)
